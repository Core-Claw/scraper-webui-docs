...
<ApiPlayground method="POST" path="/api/v1/scraper/run" />
...
| version      | 否   | string  | Worker 版本。如不填写，默认使用最新版本。 |
| input        | 是   | object  | 输入参数 |
...
| 参数                       | 示例  | 类型   | 必填 | 说明 |
| -------------------------- | ----- | ------ | ---- | ---- |
| proxy_region               | US    | string | 否   | 执行节点（ISO 3166-1 alpha-2 国家代码）。常用：`US`、`CN`、`HK`、`JP`、`SG`、`DE`、`GB`、`FR`。完整支持列表请参考 Swagger 定义。 |
