...
| 参数 | 示例                          | 类型   | 必填 | 说明              |
| ---- | ----------------------------- | ------ | ---- | ----------------- |
| slug | coreclaw/google-maps-scraper   | String | 是   | Worker 唯一标识符（即 `scraper_slug`） |
...
| version    | v1.0.5 | String | 最新版本号 |

##### `parameters.system` 字段

> **注意：** 此端点返回 `memory`，而[运行 Worker](/zh-cn/api/worker/run/) 的请求参数使用 `memory`。两者表示同一个值，单位均为 MB。

| 参数                       | 示例  | 类型    | 说明 |
| -------------------------- | ----- | ------- | ---- |
| cpus                       | 0.125 | number  | 设备 CPU 核心数 |
| memory               | 512   | Integer | 设备内存（MB） |
...
| default     | 包含 `url` 字段的对象数组  | Array   | 默认值 |
