...
将 [Windsurf](https://windsurf.com) IDE 连接到 CoreClaw MCP 服务，让您的 AI 编程助手可以在编辑器内搜索爬虫、运行任务并获取数据。
...
Windsurf 通过 **Streamable HTTP** 传输协议支持 MCP。您可以通过 MCP 设置面板进行配置。
...
3. Windsurf 应该调用 `search_scrapers` 并返回结果

## 对话示例

连接成功后，您可以要求 Windsurf 执行抓取任务：

> **您：** 找个 Google Maps 爬虫，提取纽约时代广场附近的餐厅数据。
>
> **Windsurf：** 我来搜索 Google Maps 爬虫并为您运行。*[调用 `search_scrapers` → `get_scraper_details` → `run_scraper` → 轮询状态 → 返回结果]*

## 故障排除

### 工具未出现

- **重启 Windsurf** — MCP 配置变更需要重启
- 确保 MCP 服务器 URL 正确
- 检查 `api-key` 请求头是否已设置
