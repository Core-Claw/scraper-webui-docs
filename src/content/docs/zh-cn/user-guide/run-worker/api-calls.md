...
每个套餐都对**同时**执行的运行数量有上限。通过 API 启动运行时（尤其是批量或循环调用），应遵守该限制，避免被拒绝：

- 上限只统计 `running` 状态的运行；`succeeded`、`failed` 和已取消的运行不占用。各套餐数值见[并发运行限制](/zh-cn/user-guide/run-worker/concurrency-limits/)。
- 当因达到上限导致启动运行的请求被拒绝时，响应会带有描述性提示，且**不会创建运行**——不生成 Run ID、不扣余额、不消耗运行次数。
