...
- **浏览器是远程托管的** — 无需在项目中打包浏览器，通过 `ChromeWs` 环境变量连接远程指纹浏览器池，也可以通过 `LightpandaDomain` 连接 Lightpanda（CDP/WebSocket）。详见 [浏览器指纹](/zh-cn/developer-guide/worker-definition/platform-features/browser-fingerprinting/)。
