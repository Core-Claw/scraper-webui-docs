...
try {
    ...
    await cafesdk.log.info(`浏览器认证信息: ${auth}`)
} catch (err) {
    await cafesdk.log.error(`获取浏览器认证信息失败: ${err.message}`)
    auth = null
}

let browser_url = `ws://${auth}@chrome-ws-inner.cafescraper.com`
await cafesdk.log.info(`指纹浏览器地址: ${browser_url}`)
```
...
```js
url = inputJson?.url
await cafesdk.log.info(`开始处理URL: ${url}`)
