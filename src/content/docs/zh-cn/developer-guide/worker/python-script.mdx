...
# 信息日志（正常执行流程）
CoreSDK.Log.info("成功采集 10 条新闻")

# 警告日志（非致命问题）
CoreSDK.Log.warn("检测到网络延迟，采集速度可能受影响")
...
CoreSDK.Log.info(f"开始采集网站: {website}")
...
# 4. 推送数据
CoreSDK.Log.info(f"已采集 {len(collected_data)} 条记录，正在推送数据...")
