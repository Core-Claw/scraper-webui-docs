...
| `start_time` | No | `integer` (Unix seconds) | Filter runs by `created_at` start time, Unix seconds. When provided, `end_time` is also required, and both must fall in the same calendar month. |
| `end_time` | No | `integer` (Unix seconds) | Filter runs by `created_at` end date, Unix seconds. Pass the day at `00:00:00`; the server adds 86400 seconds to include the whole day. Must be in the same calendar month as `start_time`. |
