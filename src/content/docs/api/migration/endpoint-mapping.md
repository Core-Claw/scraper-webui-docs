...
| `GET /api/store`                 | [`GET /api/v2/store`](/api/store/list/)                                                                                                  | Change `search` to `keyword`; add 1-based `offset` (page number) when paging. `limit` now defaults to 20 and cannot exceed 100.                                                                                                 |
...
| Pagination           | `page_index` starts at 1; `page_size`               | `offset` is a 1-based page index; `limit` is at most 100                        | Use `offset = page_index`.                                                            |
