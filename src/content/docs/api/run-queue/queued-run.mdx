...
| `is_async` | No | `boolean` | Accepted for API consistency but has no effect: a queued run is always placed in the queue and the response returns a `queue_ref`, never a synchronous run result. To run synchronously, use [Run Worker](/api/workers/run/) instead. |
...
| `offset` | No | `integer` | Accepted for API consistency but has no effect on the queued-run response. Constraints: default `0`; minimum 0. |
