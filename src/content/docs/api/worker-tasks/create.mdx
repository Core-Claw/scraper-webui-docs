...
| `schedule_enabled` | No | `integer` | Schedule switch: 1 enabled, 0 disabled. |
| `schedule_once_date` | No | `string` | Date for one-time schedules, `YYYY-MM-DD`. |
| `schedule_time` | No | `string` | Schedule time of day, `HH:mm`. |
| `schedule_type` | No | `integer` | Schedule type: 1=daily, 2=weekly, 3=monthly. |
| `schedule_weekday` | No | `integer` | Day of week for weekly schedules (0-6, 0 = Sunday). |
...
- Schedule fields: `schedule_enabled` 1 enabled / 0 disabled; `schedule_type` 1=daily, 2=weekly, 3=monthly; `schedule_time` is `HH:mm`; `schedule_once_date` is `YYYY-MM-DD`. Omit all schedule fields when scheduling is not needed.
