...
CoreSDK.Log.warn("Network latency detected, collection speed may be affected")

# Error logs (execution failures)
CoreSDK.Log.error("Failed to access target website, please check network connection")
```
...
config = CoreSDK.Parameter.get_input_json_dict()
website = config.get("website", "default website")
CoreSDK.Log.info(f"Starting collection for website: {website}")
...
CoreSDK.Log.info("Collecting data...")
collected_data = [
...
# 4. Push data
CoreSDK.Log.info(f"Collected {len(collected_data)} records, pushing data...")
...
# 5. Done
CoreSDK.Log.info("Data collection task completed!")
```
